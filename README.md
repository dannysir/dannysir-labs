...
`@dannysir/js-te` 의 원본은 Node.js native loader hooks(`module.registerHooks`) 위에서 동작합니다. 이 사이트의 시연은 라이브러리의 `@dannysir/js-te/browser` entry 를 위임받아 `describe`(`describe.only` / `describe.skip` 포함) / `test`(`test.each` / `test.only` / `test.skip` / `test.todo` / `test.only.each` / `test.skip.each` 포함) / `beforeEach` / `expect` / `fn()` 을 브라우저·Web Worker 에서 실행합니다. 포커스(`.only`) / 건너뜀(`.skip`) / 예정(`.todo`) 모디파이어는 0.9.0 에서, 모디파이어와 데이터주도 `.each` 의 조합(`.only.each` / `.skip.each`)은 0.9.3 에서 브라우저 entry 에 추가됐고, 데모의 `.only` / `.skip` / `.todo` / `.only.each` / `.skip.each` 예제로 시연합니다.
